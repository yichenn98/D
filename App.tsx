...
import { DrinkRecord } from './types.ts';
import { calculateStats, getFormattedDate, getTopFrequency, parseDateString } from './utils.ts';
...
const App: React.FC = () => {
  ...
  const handleDateClick = (dateStr: string) => {
    ...
    if (getRecordsForDate(dateStr).length < 2) setIsFormOpen(true);
  };

  const colors = { blue: '#DCE4E9', purple: '#E7E4ED', sage: '#E5E9E4', pink: '#EFE8E8', dot: '#A8B8A8' };

  return (
    <div className="max-w-xl mx-auto px-6 pt-[calc(1.5rem+env(safe-area-inset-top))] pb-[calc(3rem+env(safe-area-inset-bottom))] space-y-12">
      <div className="flex justify-center">
         <div className="bg-stone-600 px-14 py-6 rounded-full shadow-xl border border-stone-500 flex items-center space-x-3 transition-transform active:scale-95">
      ...
      <section className="bg-white rounded-[4rem] p-8 sm:p-10 card-shadow border border-stone-100">
        <div className="flex justify-between items-center mb-10 px-4">
          <button onClick={prevMonth} className="text-stone-300 p-2"><Icons.ChevronLeft /></button>
          <h3 className="text-2xl font-black text-stone-600 font-mono tracking-tighter">{monthLabel}</h3>
          <button onClick={nextMonth} className="text-stone-300 p-2"><Icons.ChevronRight /></button>
        </div>
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <button
                ...
                className={`relative h-16 border border-transparent flex flex-col items-center justify-center transition-all ${
                  isSelected ? 'bg-stone-100 rounded-[1.5rem] border-stone-200 z-10 scale-110 shadow-md' : 'active:bg-stone-50'
                }`}
              >
      ...
      {selectedDate && (
        <div className="space-y-6">
          <div className="bg-stone-700 p-10 rounded-[3.5rem] card-shadow text-white">
          ...
          <div className="space-y-6">
            {getRecordsForDate(selectedDate).map((r, i) => (
              <div key={r.id} className="bg-white p-8 rounded-[3rem] card-shadow border border-stone-100 flex justify-between items-center">
                <div className="space-y-1.5 flex-1 pr-4">
                  ...
                  <p className="text-sm text-stone-500 font-bold">甜度：{r.sweetness}；冰塊：{r.ice}</p>
                </div>
                <div className="flex items-center space-x-4">
                   <span className="text-3xl font-black text-stone-700 tracking-tighter">${r.price}</span>
                   <button onClick={(e) => { e.stopPropagation(); removeRecord(r.id); }} className="p-3 text-stone-200"><Icons.Trash /></button>
                </div>
              </div>
            ))}

            {getRecordsForDate(selectedDate).length < 2 && (
              <button onClick={() => setIsFormOpen(true)} className="w-full py-10 border-2 border-dashed border-stone-200 rounded-[3rem] text-stone-400 flex items-center justify-center space-x-4 bg-white/40 active:bg-stone-50">
                <div className="p-3 rounded-full bg-stone-100"><Icons.Plus /></div>
      ...
      <section className="grid grid-cols-2 gap-6">
        <button onClick={() => setAnalyticsType('shop')} className="bg-white p-8 rounded-[3rem] card-shadow border border-stone-100 space-y-3 text-left active:scale-95">
        ...
        <button onClick={() => setAnalyticsType('item')} className="bg-white p-8 rounded-[3rem] card-shadow border border-stone-100 space-y-3 text-left active:scale-95">
      ...
      {analyticsType && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/40 backdrop-blur-xl p-8">
          <div className="w-full max-w-lg bg-white rounded-[3.5rem] p-10 card-shadow max-h-[80vh] flex flex-col">
